...
func (hc *handlerConfig) count(incr int) int {
	hc.counterMu.Lock()
...
func main() {
	...
	genReader := func(reader io.Reader) singleHandler {
		return func() (data string, n int, err error) {
			...
			if !ok {
				err = errors.New("upsupported reader")
				return
			}
			mu.Lock()
			defer mu.Unlock()
	...
	sign := make(chan struct{}, signNumber)

	//启用多个gorouting对缓冲区进行多次数据写入
