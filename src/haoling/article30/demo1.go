...
func main() {
	...
	box2.Store(v2) //这里不会引发panic，因为box2在这之前并没有被真正的使用
	fmt.Printf("The value load from box is %v \n", box.Load())
	...
	if ok {
		...
		//box4.Store(v42) // 这里会引发一个panic，报告存储值的类型不一致，即使是接口的实现类型也会被判定类型不同而引发panic
	}
	fmt.Println()

	//封装一个结构体，利用反射判断存储的类型是否和之前相同
	...
	//不要直接将引用类型的变量store至atomic.Value
