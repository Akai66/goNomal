...
func main() {
	...
	reader2 := io.LimitReader(reader1, num1) //LimitReader类型，指定最多读取的字节上限
	buf2 := make([]byte, 10)
	for i := 0; i < 3; i++ {
	...
	reader3 := io.NewSectionReader(reader1, start, total) //从start开始，最多只能读total个字节
	buf3 := make([]byte, 20)
	for i := 0; i < 5; i++ {
	...
	reader4 := io.TeeReader(reader1, writer1) //TeeReader的Read方法会把reader1中的数据经过作为方法参数的字节切片buf4写入writer，可以说这个值就是r和w之间的数据桥梁，而buf4就是这座桥梁上的数据搬运者
	buf4 := make([]byte, 40)
	for i := 0; i < 8; i++ {
